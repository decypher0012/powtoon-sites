...
The current workspace workflow adds a Quick Access strip for pinned, recent, and frequently launched items; reusable preset templates; per-preset global hotkeys; startup-readiness checks; timed focus sessions; preset chaining; and per-item weekday, network, and time-window conditions. Preset runs continue to produce a launch summary with elapsed times and retry actions. Configure these optional controls in **Manage Workspaces > Presets**; advanced settings stay out of the primary dashboard until they are used.

Workspace lifecycle controls can capture the currently visible Windows application layout into a preset, restore recorded window rectangles after launch, and end applications started by an active workspace. Presets can also define work hours, open Windows Focus, prompt for runtime placeholders such as `{client}`, keep secret parameter values in a separate Windows DPAPI-protected file, check or install reviewed WinGet package IDs, open an imported browser session, and run explicitly configured `open_uri`, Windows Terminal, Remote Desktop, or executable actions. The optional always-on-top Mini Dock exposes pinned presets without keeping the main window open.

The opt-in `browser-extension/` companion exports HTTP/HTTPS tabs from the current Chromium browser window to a local JSON file. Use **Tools > Export Browser Companion**, load the exported folder as an unpacked extension from the browser's extension management page, choose **Export current window**, then use **Tools > Import Browser Session**. The extension does not transmit browsing data, read page contents, or access credentials.
